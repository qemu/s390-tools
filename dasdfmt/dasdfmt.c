 ...
 */
static int check_param(char *s, format_data_t *data)
{
	int tmp = data->blksize;

	if ((tmp < 512) || (tmp > 4096)) {
		strcpy(s,"Blocksize must be one of the following positive "
		       "integers:\n512, 1024, 2048, 4096.");
		return -1;
	}
				 ...
				 unsigned int cylinders, unsigned int heads)
{
	...
	rc = write(filedes, &ipl1, sizeof(bootstrap1_t));
	if (rc != sizeof(bootstrap1_t))
	...
	rc = write(filedes, &ipl2, sizeof(bootstrap2_t));
	if (rc != sizeof(bootstrap2_t))
			   ...
			   unsigned int heads, format_data_t *format_params)
{
	...
	while (1) {
		...
		format_step.stop_unit  = k + heads - 1;
		...
		if (info->print_progressbar) {
			printf("cyl %5d of %5d |", cyl, cylinders);
			p2 = p1;
