 ...
 *
 * Returns 1 if @msg contains the escape charater @esc only; otherwise the
 * function returns 0.
 */
 ...
 * @esc:	The escape character
 *
 * Returns the appropriate action of the escaped charater.
 ...
 */
static int iucvtty_worker(int terminal, const struct iucvtty_cfg *cfg)
{
	...
	while (1) {
		...
		if (FD_ISSET(terminal, &set)) {
			...
			case MSG_TYPE_DATA:
				...
				break;
			case MSG_TYPE_ERROR:
				iucvtty_error(*(uint32_t *) msg->data);
				break;
			}
		}
