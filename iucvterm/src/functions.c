 ...
 *
 * The return code is identical to strmatch(). The function returns always
 * zero if client_checking is disabled by the configuration.
 */
int is_client_allowed(const char *client, const struct iucvtty_cfg *cfg)
{
	if (!cfg->client_check)
		return 0;
