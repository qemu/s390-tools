	    ...
	    struct disk_info* info)
{
	...
	rc = boot_init_fba_stage0(&stage0, stage2_list, stage2_count);
	if (rc == 0) {
		/* Store program table pointer in stage 0 loader */
		bootmap_store_blockptr(&stage0.param1, program_table, info);
		/* Install stage 0 */
		rc = DRY_RUN_FUNC(misc_write(
			fd, &stage0, sizeof(struct boot_fba_stage0)));
	}
	if (rc)
		     ...
		     struct disk_info* info)
{
	...
	/* Install stage 0 */
	rc = DRY_RUN_FUNC(misc_write(fd, &stage0,
	...
	rc = DRY_RUN_FUNC(misc_write(fd, &stage1,
				  ...
				  struct disk_info* info)
{
	...
	param->end_blk = info->geo.start + info->phy_blocks - 1;
	param->num_heads = info->geo.heads;
	param->blocksize = info->phy_block_size;
	param->bpt = disk_sec_from_blocknum(param->end_blk, info);
}
...
install_dump_eckd_classic(int fd, struct disk_info* info, uint64_t mem)
{
	...
	if (rc)
		return rc;
	/* Fill in dump partition parameter */
	eckd_dump_store_param((struct eckd_dump_param *) &stage1.param1, info);
	/* Install stage 1 */
	if (lseek(fd, info->phy_block_size, SEEK_SET) !=
	    info->phy_block_size) {
		error_reason(strerror(errno));
...
install_dump_eckd_compatible(int fd, struct disk_info* info, uint64_t mem)
{
	...
	/* Prepare stage 1 */
	rc = boot_init_eckd_compatible_dump_stage1(&stage1, info, 0);
	if (rc)
		return rc;
	/* Fill in dump partition parameter */
	eckd_dump_store_param((struct eckd_dump_param *) &stage1.param1, info);
	...
	free(buffer);
	return overwrite_partition_start(fd, info, 0);
}


static int
install_mvdump_eckd_compatible(int fd, struct disk_info* info, uint64_t mem,
			       uint8_t force, struct mvdump_parm_table parm)
{
	...
	/* Prepare stage 1 */
	rc = boot_init_eckd_compatible_dump_stage1(&stage1, info, 1);
	if (rc)
		return rc;
	/* Install stage 1 */
	if (lseek(fd, info->phy_block_size + 4, SEEK_SET) !=
	...
	rc = boot_get_eckd_mvdump_stage2(&buffer, &size, mem, force, parm);
	if (rc)
		return rc;
	rc = DRY_RUN_FUNC(misc_write(fd, buffer, size));
	free(buffer);
	...
	return overwrite_partition_start(fd, info, 1);
}
...
install_dump_fba(int fd, struct disk_info* info, uint64_t mem)
{
	...
	if (rc)
		return rc;
	/* Install stage 2 dump record at end of partition */
	rc = boot_get_fba_dump_stage2(&buffer, &size, mem);
	if (rc)
		return rc;
	...
	block = info->geo.start + info->phy_blocks - count;
	if (lseek(fd, block * info->phy_block_size, SEEK_SET) !=
	    (off_t) block * info->phy_block_size) {
		error_reason(strerror(errno));
		error_text("Could not seek on device");
		free(buffer);
		return -1;
	...
	for (i=0; i < count; i++) {
		stage2_list[i].linear.block = block + i;
		stage2_list[i].linear.size = info->phy_block_size;
		stage2_list[i].linear.blockct = 0;
	}
	/* Prepare stage 0 */
	rc = boot_init_fba_stage0(&stage0, stage2_list, count);
	if (rc)
		return rc;
	/* Fill in dump parameters */
	stage0.param1 = (uint64_t) info->geo.start;
	stage0.param2 = (uint64_t) block - 1;
	/* Install stage 0 */
	if (lseek(fd, 0, SEEK_SET) != 0) {
		error_reason(strerror(errno));
		error_text("Could not seek on device");
...
install_dump(const char* device, struct job_target_data* target, uint64_t mem)
{
	...
	case disk_type_fba:
		...
		printf("Dump target: partition '%s' with a size of "
		       "%ld MB.\n", device, (long) part_size >> 20);
		...
		else if (info->type == disk_type_eckd_compatible)
	       ...
	       uint64_t mem, uint8_t force)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (info[i]->type != disk_type_eckd_compatible) {
		...
		parm.param[i].end_blk = info[i]->geo.start +
					info[i]->phy_blocks - 1;
		parm.param[i].end_sec =
			disk_sec_from_blocknum(info[i]->geo.start +
					       info[i]->phy_blocks - 1,
					       info[i]);
	...
	for (i = 0; i < count; i++) {
		...
		if (verbose)
			printf("Installing dump record on target partition "
			       "'%s'\n", device[i]);
		rc = install_mvdump_eckd_compatible(fd, info[i], mem, force,
						    parm);
		misc_free_temp_dev(tempdev);
		close(fd);
		if (rc)
