...
static int get_ur_devno(int minor)
{
	...
	if (errno != 0)
		...
		ERR_EXIT("Device not online\n");
found:
	if (sscanf(bus_id, "0.0.%x", &devno) != 1)
		ERR_EXIT("Could not extract devno from %s\n", direntp->d_name);
	closedir(fd);
	return devno;
}
...
static void parse_opts_receive(struct vmur *info, int argc, char *argv[])
{
	...
	static const char option_string[] = "vhtOfHcd:b:";
...
static void parse_opts_punch_print(struct vmur *info, int argc, char *argv[])
{
	...
	static const char option_string[] = "vhtrfu:n:d:b:N:";
...
static void parse_opts_purge(struct vmur *info, int argc, char *argv[])
{
	...
	static const char option_string[] = "fvhq:";
...
/*
 * Get filename for spoolid from reader
 */
...
static void acquire_lock(struct vmur *info)
{
	...
	fd = open(LOCK_FILE, O_RDONLY | O_CREAT, S_IRUSR);
	if (fd == -1) {
...
/*
 * Convert record for texmode: Do EBCDIC->ASCII translation
 */
...
static void ur_receive(struct vmur *info)
{
	...
	if (check_class(info))
		ERR_EXIT("Reader device class does not match spool file "
			 "class.\nIssue CP command 'SPOOL %X CLASS *' to "
			 "enable %X for any spool file class.\n",
			 info->devno, info->devno);
	acquire_lock(info);
	close_reader(info, "HOLD");
	...
	/* Read first block and check spoolfile format */
	...
	if (info->convert_specified) {
		if (type != TYPE_VMDUMP) {
			...
			close(fhi);
			if (info->stdout_specified)
				rc = vm_convert(info->devnode, NULL, prog_name);
			else
				rc = vm_convert(info->devnode, info->file_name,
						prog_name);
			if (rc)
				goto fail;
			else
...
static void ur_purge_question(struct vmur *info)
{
	...
	if (info->spoolid_specified)
		fprintf(stderr, "%s: purge %s %s? ", prog_name, info->queue,
		       info->spoolid);
	else
		fprintf(stderr, "%s: purge all %s files? ", prog_name,
			info->queue);
	str = fgets(buf, sizeof(buf), stdin);
	if (!str)
		exit(1);
...
static int ur_purge(struct vmur *info)
{
	...
	if (info->spoolid_specified)
		sprintf(cmd, "PURGE * %s %s", info->queue, info->spoolid);
	else
		sprintf(cmd, "PURGE * %s ALL", info->queue);
	acquire_lock(info);
	cpcmd(cmd, &buf, NULL, 0);
	ERR("%s", buf);
	free(buf);
