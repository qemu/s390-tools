...
/**********************************************************************
 * oid_to_str_conv():
 *  This function converts a ucd-snmp OID u_long array into an OID        
...
int search_oid ( oid* s_oid, size_t len, TABLE_OID* lhead, TABLE_OID** curr  )
{
  ...
  /* fyi - snmp_oid_compare() is a function from the ucd-snmp agent extension API */
		     ...
		     TABLE_OID* lhead, TABLE_OID** curr  )
{
  ...
  /* loop through list and compare OIDs */
  /* snmp_oid_compare() is a taken from the ucd-snmp agent extension API */
  for( *curr=lhead; (*curr)->next != NULL; *curr=(*curr)->next )    
    {
...
int delete_oid( oid* d_oid, size_t len, TABLE_OID* lhead )
{
  ...
  /* loop through list and compare OIDs  */
  /* snmp_oid_compare() is taken from the ucd-snmp agent extension API */
  for( curr=lhead; curr->next != NULL; curr=curr->next )    
    {
...
int search_index ( char* s_index, REG_INDICES* lhead, REG_INDICES** curr  )
{
  ...
  oid  ind_oid1[MAX_OID_LEN];           /* temporary ucd-snmp oids */
 ...
 *  suffixes with ucd-snmp subagent driving code using the REGISTER_MIB
...
{ 
  ...
  oid  t_oid[MAX_OID_LEN];           /* temporary ucd-snmp oid */
  oid  *top_oid;
  char *new_index;                         
  ...
  REG_INDICES    *ind_ptr, *ins_ind; /* ptr into index linked list */
 
  /* ptr to ucd-snmp variable13 structs that will contain the suffix
  ...
  for ( i=0; i < mib_data_hdr->table_cnt; i++ ) {
    ...
    /* convert string OID to ucd-snmp oid type */
	...
	mib_data_ptr += sizeof( int );    
	  ...
	  if ( (strcmp( last_suffix, (char*) mib_data_ptr ) !=0)  || j==0 )
		...
		/* reassign table_vars */
	      ...
	      /* convert suffix OID string to ucd-snmp oid type */
	      oid_len = str_to_oid_conv( (char*) mib_data_ptr, t_oid ); 
	      if ( oid_len == 0 ) {
		get_time( time_buf );      
		snmp_log( LOG_ERR, "%s register_tables(): "
	...
	src_ind = search_index( (char*) mib_data_ptr, ins_oid->ind_list, &ind_ptr );
	  ...
	  ins_ind = index_insert_after( new_index,
    ...
    /* note - register_mib() is taken from the ucd-snmp agent extension API */
 ...
 * Return matching ucd-snmp ifIndex to caller.
 * 
 * parameters:
  ...
  if ( exact && res == 0 )
	...
	index_len = (int) (*length) - (int) vp->namelen;
      ...
      DEBUGMSG(("ibmOSAMib-Subagent", 
		  " header_osa_table - GET index length: %d\n", index_len));
