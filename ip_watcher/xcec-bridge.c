...
#define PROC_QETH_FILE "/proc/qeth"
#define PROC_QETH_FILE_WIDTH 100
...
int read_proc(struct int_sock **nlist)
{
	FILE *fh;
	char buf[PROC_QETH_FILE_WIDTH];
	int retval,line_no = 2;
	char r_dev_name[PROC_QETH_FILE_WIDTH];
	char r_rtr[PROC_QETH_FILE_WIDTH];
	int r;
	struct int_sock *i = NULL;

	fh=fopen(PROC_QETH_FILE,"r");
	if (!fh) {
		syslog(LOG_ERR,"can't open %s -- won't use different " \
		       "interfaces",PROC_QETH_FILE);
		return 1;
	}
	/* read the two header lines */
	if ( (!fgets(buf,PROC_QETH_FILE_WIDTH,fh)) ||
	     (!fgets(buf,PROC_QETH_FILE_WIDTH,fh)) ) {
		syslog(LOG_ERR,"can't read header from %s -- "
		       "won't use different interfaces",PROC_QETH_FILE);
		fclose(fh);
		return 1;
	}
	...
	do {
		...
		strcpy(r_dev_name,"");
		/* format string: devnos chpid dev_name
		 * dev_name is what we're interested in */
		retval=fscanf(fh,"%*s %*s %s %*s %*s %*s %*s %s",
			      r_dev_name,r_rtr);
		if (retval==EOF) {
			break;
		}
		...
		/* dull right now, but maybe useful in the future */
		if (retval!=2) {
			syslog(LOG_ERR,"can't interpret line %i from " \
			       "%s -- some interface might not be used",
			       line_no,PROC_QETH_FILE);
			continue;
		}

		/* we only want to forward between affected interfaces:
		 * mc, pri, sec, p.c, s.c, mix */
		if (!(
		      (!strcmp(r_rtr,"mc"))||
		      (!strcmp(r_rtr,"mc+"))||
		      (!strcmp(r_rtr,"p.c"))||
		...
		i=malloc(sizeof(struct int_sock));
		if (!i) {
			syslog(LOG_ERR,"no memory while reading from %s -- " \
				       "some interface might not be used",
				       PROC_QETH_FILE);
			continue;
		}

		/* as soon as we have one interface echoeing back broadcasts
		 * to us, we don't bridge broadcast traffic */
		if (!(
		      (!strcmp(r_rtr,"mc+"))||
		      (!strcmp(r_rtr,"p+c"))||
		      (!strcmp(r_rtr,"s+c"))
		...
		i->mtu_warning=0;
		strncpy(i->dev_name,r_dev_name,DEV_NAME_LEN);
		if (!strncmp(r_dev_name,"hsi",3)) {
			i->features=I_S_FEATURE_PASSTHROUGH;
		}
...
void update_interfaces()
{
	...
	rc = stat(PROC_QETH_FILE, &st);
	if (rc == 0) {
		if (read_proc(&new_list))
			return;
	} else {
